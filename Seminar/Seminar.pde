/**
 * 船が川の両岸で跳ね返りながら、画面左端から右端・右端から左端に移動するアニメーション
 */
...
void draw() {
  ...
  // 船の座標を移動
  x += variation * cos(theta);
  y += variation * sin(theta);
  
  if (moveToTheRight && x > width + 45) {
    moveToTheRight = false;
    variation = -2;
  } else if (!moveToTheRight && x <= 0) {
    moveToTheRight = true;
    variation = 4;
  }
  ...
  translate(x, y);
  rotate(theta);
  if (moveToTheRight == false) scale(-1, 1); // 左右反転
